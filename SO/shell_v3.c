...
int main() {
    ...
    while (1) { // Mantiene el programa en ejecución para procesar comandos continuamente.
        ...
        for (int i = 0; i < cantidad_comandos; i++) {
            int entrada_fd = (i == 0) ? STDIN_FILENO : tuberias[i - 1][0]; // Define la entrada estándar o la tubería anterior.
            int salida_fd = (i == cantidad_comandos - 1) ? STDOUT_FILENO : tuberias[i][1]; // Define la salida estándar o la siguiente tubería.
